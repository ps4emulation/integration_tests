...
namespace FS_Test
{
    ...
    {
        OrbisKernelStat st;
        if (int status = sceKernelFstat(fd, &st); status < 0)
            return status;
        return st.st_size;
    }
    ...
    {
        OrbisKernelStat st;
        if (int status = sceKernelStat(path, &st); status < 0)
            return status;
        return st.st_size;
    }
    ...
    void Obliterate(const char *path)
    {
        ...
        for (auto it = entries.rbegin(); it != entries.rend(); ++it)
        {
            ...
            // see what sticks
            errno = 0;
            if (0 == sceKernelUnlink(pp))
                continue;
            errno = 0;
            if (0 == sceKernelRmdir(pp))
                continue;
            LogError("Cannot remove [", pp, "] ( errno =", errno, ")");
        }
        if (0 != sceKernelRmdir(path))
            LogError("Cannot remove [", path, "] ( errno =", errno, ")");
    ...
    void ElEsDashElAy(const char *path)
    {
        ...
        for (const auto &entry : fs::directory_iterator(path, ec))
        {
            ...
            Log(file_mode(st.st_mode), right('0' + to_octal(st.st_mode), 8), std::dec, right(STR(st.st_nlink), 3), st.st_uid, ":", st.st_gid, right(STR(st.st_size), 8), timebuf, pathstr);
    ...
    u16 dumpDirRecursive(fs::path path, int depth)
    {
        ...
        while (read_bytes > 0)
        {
            ...
            while (bufptr < endptr)
            {
                ...
                {
                default:
                    ftype = std::to_string(entry->d_type);
                    break;
                case 2:
                    ftype = "DEV";
                    break;
    ...
    s8 GetDir(fs::path path, fs::path leaf, oi::PfsDirent *dirent)
    {
        ...
        while (read_bytes > 0 && !found)
        {
            ...
            while (bufptr < bufend)
            {
                ...
                if (entry->d_namlen != target_file_name_length)
                    continue;
            ...
            if (read_bytes < DIRENT_BUFFER_SIZE)
                break;
