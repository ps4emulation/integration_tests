...
namespace FS_Test
{
    ...
    void RunTests()
    {
        ...
        TEST_CASE(TestStat("/dev/stdin", &DumpedConstants::stat_root_dev_stdin), "Test complete [stat should fail]", "Stat comparsion failed");
        TEST_CASE(!TestStat("/dev/stdout", &DumpedConstants::stat_root_dev_stdout), "Test complete", "Stat worked on /dev/stdout");
        ...
        if (cursed_fileop_prepared)
        {
            ...
            TEST_CASE(TestFileTouch("/data/therapist/tmp_cursed/Aursed/AAursed/../../Cursed/CCursed/../../Bursed/BBursed/a_aa_dd_dd_c_cc_dd_dd_b_bb_file.txt"), "Test complete", "Test failed");
            Log("errno for tests below should equal 9 (EBADF) and 2 (ENOENT)");
            TEST_CASE(!TestFileTouch("/data/therapist/tmp_cursed/../tmp_cursed/../../././data/therapist/../data/therapist/././tmp_cursed/Cursed/../../../data/therapist/tmp_cursed/Cursed/idfk.txt"), "Test complete", "Test failed");
            Log("errno for tests below should equal 9 (EBADF) and 22 (EINVAL)");
            TEST_CASE(!TestFileTouch("../../../data/therapist/tmp_cursed/escape_from_app0_file.txt"), "Test complete: Can't", "Test failed: Can", "escape from curdir with relatives");
        ...
        if (file_open_tests_prepared)
        {
            ...
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_ro.txt", O_RDONLY, "O_RDONLY");
                 _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            // O_WRONLY
            ...
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rowo.txt", O_RDONLY | O_WRONLY | O_RDWR, "O_RDONLY | O_WRONLY | O_RDWR");
                 _errno == 22, "Pass (EINVAL)", "Fail", "( errno =", _errno, "should be =", 22, ")");
            ...
            // O_CREAT
            // these create a file
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_roc.txt", O_RDONLY | O_CREAT, "O_RDONLY | O_CREAT"); _errno == 0, "Pass", "Fail", "( errno =", _errno, "should be =", 0, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_woc.txt", O_WRONLY | O_CREAT, "O_WRONLY | O_CREAT"); _errno == 0, "Pass", "Fail", "( errno =", _errno, "should be =", 0, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rwc.txt", O_RDWR | O_CREAT, "O_RDWR | O_CREAT"); _errno == 0, "Pass", "Fail", "( errno =", _errno, "should be =", 0, ")");
            // O_CREAT | O_EXCL
            // exists, not creating
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_roc.txt", O_RDONLY | O_CREAT | O_EXCL, "O_RDONLY | O_CREAT | O_EXCL"); _errno == 17, "Pass (EEXIST)", "Fail", "( errno =", _errno, "should be =", 17, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_woc.txt", O_WRONLY | O_CREAT | O_EXCL, "O_WRONLY | O_CREAT | O_EXCL"); _errno == 17, "Pass (EEXIST)", "Fail", "( errno =", _errno, "should be =", 17, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rwc.txt", O_RDWR | O_CREAT | O_EXCL, "O_RDWR | O_CREAT | O_EXCL"); _errno == 17, "Pass (EEXIST)", "Fail", "( errno =", _errno, "should be =", 17, ")");
            // O_APPEND
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_roa.txt", O_RDONLY | O_APPEND, "O_RDONLY | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_woa.txt", O_WRONLY | O_APPEND, "O_WRONLY | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rwa.txt", O_RDWR | O_APPEND, "O_RDWR | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            // O_TRUNC | O_APPEND
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rota.txt", O_RDONLY | O_TRUNC | O_APPEND, "O_RDONLY | O_TRUNC | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_wota.txt", O_WRONLY | O_TRUNC | O_APPEND, "O_WRONLY | O_TRUNC | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_rwta.txt", O_RDWR | O_TRUNC | O_APPEND, "O_RDWR | O_TRUNC | O_APPEND"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            // O_DIRECTORY (nonexistent file is a target)
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_dir_ro.txt", O_RDONLY | O_DIRECTORY, "O_RDONLY | O_DIRECTORY"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_dir_wo.txt", O_WRONLY | O_DIRECTORY, "O_WRONLY | O_DIRECTORY"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            TEST_CASE(int _errno = TestOpenFlags("/data/therapist/tmp_open/nonexistent_dir_rw.txt", O_RDWR | O_DIRECTORY, "O_RDWR | O_DIRECTORY"); _errno == 2, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 2, ")");
            ...
            TEST_CASE(int _errno = TestOpenFlags("assets/misc/file.txt", 0, "O_RDONLY");
                 _errno == 22, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 22, ")");
            TEST_CASE(int _errno = TestOpenFlags("./assets/misc/file.txt", 0, "O_RDONLY");
                 _errno == 22, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 22, ")");
            TEST_CASE(int _errno = TestOpenFlags("", 0, "O_RDONLY");
                 _errno == 22, "Pass (ENOENT)", "Fail", "( errno =", _errno, "should be =", 22, ")");
        ...
        if (file_rw_tests_prepared)
        {
            ...
            TEST_CASE(!TestFileRW("/app0/assets/misc/file_empty.txt", 32), "Test complete:", "Test failed:", "[R/W on a file in RO directory should fail with errno = 30 (EROFS)]");
        ...
        TEST_CASE(stat(case_insensitive_path_lower, &st) != 0 && errno == ENOENT, "Data: Data: Lowercase sensitivity passed", "Data: Resolved name in lowercase", "( ", case_insensitive_path_lower, " )");
        TEST_CASE(stat(case_insensitive_path_upper, &st) != 0 && errno == ENOENT, "Data: Uppercase sensitivity passed", "Data: Resolved name in uppercase", "( ", case_insensitive_path_upper, " )");
        TEST_CASE(stat(case_insensitive_path_upper_end, &st) != 0 && errno == ENOENT, "Data: Uppercase sensitivity (2nd half) passed", "Data: Resolved name (2nd half) in uppercase", "( ", case_insensitive_path_upper_end, " )");
        ...
        TEST_CASE(stat(case_insensitive_path_app0_lower, &st) == 0, "app0: Data: Lowercase sensitivity passed", "app0: Can't resolve name in lowercase", "( ", case_insensitive_path_app0_lower, " )");
        TEST_CASE(stat(case_insensitive_path_app0_upper, &st) != 0 && errno == ENOENT, "app0: Uppercase (whole path) sensitivity passed", "app0: Resolved (whole path) name in uppercase", "( ", case_insensitive_path_app0_upper, " )");
        TEST_CASE(stat(case_insensitive_path_app0_upper_end, &st) == 0, "app0: Uppercase (app0 path) sensitivity passed", "app0: Can't resolve (app0 path) name in uppercase", "( ", case_insensitive_path_app0_upper_end, " )");
    }
    ...
    bool TestFileOps(const char *path)
    {
        ...
        // write test string
        errno = 0;
        TEST_CASE(int bw = sceKernelWrite(fd, teststr, teststr_len); bw == teststr_len, "Write successful", "Write failed", "(Written:", bw, ") should write", teststr_len, "( errno =", errno, ")");
        // check ptr after writing
        errno = 0;
        TEST_CASE(int curp = sceKernelLseek(fd, 0, 1); curp == teststr_len, "ptr is valid", "ptr is invalid", "(lseek is", curp, "should be:", teststr_len, ") ( errno =", errno, ")");
        // return to origin
        errno = 0;
        ...
        TEST_CASE(int br = sceKernelRead(fd, readbuffer, 256); br == teststr_len, "Read successful", "Read failed", "(Read:", br, "should read:", teststr_len, ") ( errno =", errno, ")");
        Log("\t", readbuffer);
        errno = 0;
        ...
        errno = 0;
        TEST_CASE(int newp = sceKernelLseek(fd, 0, 1); newp == 38, "file pointer not changed after resizing", "file ptr changed after resizing", "(lseek is", newp, "should be", 38, ") ( errno =", errno, ")");
        ...
        TEST_CASE(int bw = sceKernelWrite(fd, teststr2 + 20, teststr2_len - 20); bw == teststr2_len - 20, "2-nd write successful", "2-nd write failed", "(Written:", bw, ") should write", teststr2_len - 20, "( errno =", errno, ")");
        errno = 0;
        // good advice
        TEST_CASE(int curp = sceKernelLseek(fd, 0, 0); curp == 0, "ok:", "fail:", "origin+0 (lseek is", curp, "should be:", 0, ") ( errno =", errno, ")");
        errno = 0;
        memset(readbuffer, 0, 256);
        TEST_CASE(int br = sceKernelRead(fd, readbuffer, 51); br == 51, "Read successful", "Read failed", "(Read:", br, "should read:", 51, ") ( errno =", errno, ")");
        Log("\t", readbuffer);
        // last element
        errno = 0;
        ...
        errno = 0;
        memset(readbuffer, 0, 256);
        TEST_CASE(int br = sceKernelRead(fd, readbuffer, 256); br == 24, "Read successful", "Read failed", "(Read:", br, "should read:", 24, ") ( errno =", errno, ")");
        Log("\t", readbuffer);
        ...
        errno = 0;
        // absolute, 2137
        TEST_CASE(int curp = sceKernelLseek(fd, 2137, 0); curp == 2137, "ok:", "fail:", "origin+2137 (OOB) (lseek is", curp, "should be:", 2137, ") ( errno =", errno, ")");
        errno = 0;
        ...
        TEST_CASE(int curp = sceKernelLseek(fd, 2137, 1); curp == 2137 * 2, "ok:", "fail:", "current+2137 (OOB) (lseek is", curp, "should be:", 4274, ") ( errno =", errno, ")");
        errno = 0;
        // n to the back of current ptr, 2137+2137-1234
        TEST_CASE(int curp = sceKernelLseek(fd, -1234, 1); curp == 3040, "ok:", "fail:", "current-1234 (OOB) (lseek is", curp, "should be:", 3040, ") ( errno =", errno, ")");
        errno = 0;
        ...
        TEST_CASE(int curp = sceKernelLseek(fd, 2137, 2); curp == oob_end_pos, "ok:", "fail:", "end+2137 (OOB) (lseek is", curp, "should be:", oob_end_pos, ") ( errno =", errno, ")");
        errno = 0;
        // error, no change in ptr
        TEST_CASE(sceKernelLseek(fd, -2137, 2) < 0 && errno == 22, "ok:", "fail:", "end-2137 (OOB) (errno should be 22 - EINVAL) ( errno =", errno, ")");

        if (0 != sceKernelClose(fd))
            LogError("Can't close [", path, "]", "( errno =", errno, ")");
    ...
    bool TestFileRW(const char *path, u16 to_test)
    {
        ...
        if (bw > 0)
            Log("Write preview:", right(STR(writebuf[0]), 3), right(STR(writebuf[1]), 3), right(STR(writebuf[2]), 3), right(STR(writebuf[3]), 3));
        if (br > 0)
            Log("Read  preview:", right(STR(readbuf[0]), 3), right(STR(readbuf[1]), 3), right(STR(readbuf[2]), 3), right(STR(readbuf[3]), 3));
    ...
    bool TestDirEnts()
    {
        ...
        errno = 0;
        TEST_CASE(sceKernelMkdir("/data/therapist/tmp_dirent", 0777) == 0x80020011, "Creating [ /data/therapist/tmp_dirent ] failed (EEXIST)", "Created [ /data/therapist/tmp_dirent ] even though it existed before", "( errno =", errno, ")");
        ...
        errno = 0;
        TEST_CASE(int status = sceKernelGetdirentries(fd, prebuffer, DIRENT_BUFFER_SIZE, nullptr); status == DIRENT_BUFFER_SIZE, "Direntries read correctly", "Can't get direntries", "( read bytes:", status, ")", "( errno =", errno, ")");
        errno = 0;
        TEST_CASE(sceKernelLseek(fd, 0, 0) == 0, "Seek to start", "Seek failed", "(dump, pre)", "( errno =", errno, ")");
        errno = 0;
        TEST_CASE(int last_reclen = dumpDir(fd); last_reclen == (DIRENT_BUFFER_SIZE - 96), "Read correct amount of data", "Last dirent has incorrect value", "(dump, pre)", "( last_reclen =", last_reclen, "should be =", DIRENT_BUFFER_SIZE - 96, ")", "( errno =", errno, ")");
        ...
        errno = 0;
        TEST_CASE(int status = sceKernelGetdirentries(fd, postbuffer, DIRENT_BUFFER_SIZE, nullptr); status == DIRENT_BUFFER_SIZE, "Direntries read correctly", "Can't get direntries", "( read bytes:", status, ")", "( errno =", errno, ")");
        errno = 0;
        TEST_CASE(sceKernelLseek(fd, 0, 0) == 0, "Seek to start", "Seek failed", "(dump, post)", "( errno =", errno, ")");
        errno = 0;
        TEST_CASE(int last_reclen = dumpDir(fd); last_reclen == (DIRENT_BUFFER_SIZE - 192), "Read correct amount of data", "Last dirent has incorrect value", "(dump, post)", "( last_reclen =", last_reclen, "should be =", DIRENT_BUFFER_SIZE - 192, ")", "( errno =", errno, ")");
